    ...
    function render(){
      ...
      function getPlayerName(playerSymbol){
        if(playerSymbol === state.players[0].symbol)
          return state.players[0].isComputer ? 'Computer' : "Player 1"
        return state.players[1].isComputer ? 'Computer' : "Player 2"
      }
      ...
      function htmlQ1(){
        return `<div id="view1"><h3>Which game do you prefer?\n</h3>
        ${buttonHTML(1, "1 player", "Human vs Computer")}
        ${buttonHTML(1, "2 players", "Multiplayer")}
        </div>`
      }
  
      function htmlQ2(){
        const html2=`<div id="view2"><h3>${!state.players[1].isComputer? "Player 1, <br />" : ""}Which symbol would you like to use?</h3>
        ${buttonHTML(2, "X", "X")}
        ${buttonHTML(2, "O", "O")}`
        return html2
      }
      ...
      function htmlGame (){
        ...
        const playerName = state.game.turn === 0 ? 'Player 1' : state.players[1].isComputer ? 'Computer' : 'Player 2'
